...
export default function FoodCategory ({data, selected}) {
    ...
    if ('categories' in data) {
        ...
        return (
            <> 
                ...
                {isOpen && data.categories.map((item) => ( <FoodCategory key={item.title} data={item} selected={selected} />))}
            </>
        )
    }
    ...
    let filterData;
    if (selected === 'veg') {
        filterData = data.itemCards.filter((ele) => (
            ele?.card?.info?.itemAttribute?.vegClassifier === "VEG"
        ))
    }
    else if (selected === 'nonveg') {
        filterData = data.itemCards.filter((ele) => (
            ele?.card?.info?.itemAttribute?.vegClassifier === "NONVEG"
        ))
    }
    else {
        filterData = data?.itemCards;
    }
        ...
        <div className=" w-200 mx-auto flex flex-col gap-4">
            <div className="p-4">
                ...
                <div className="flex justify-between" onClick={() => setIsOpen(!isOpen)}>
                    <p className="font-bold text-[1.15rem] mb-6">{data?.title} {"(" + filterData?.length + ")"}</p>
                ...
                {
                isOpen && filterData?.map((item) => (<FoodCategoryCard key={item?.card?.info?.id} data={item?.card?.info} />))
                }
